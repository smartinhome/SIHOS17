...
"<button class='tab active' data-page='dashboard' onclick='showPage(\"dashboard\",this)'>Dashboard</button>"
"<button class='tab' data-page='wifi' onclick='showPage(\"wifi\",this)'>WiFi</button>"
"<button class='tab' data-page='update' onclick='showPage(\"update\",this)'>Aktualizacja</button>"
"<button class='tab' data-page='logs' onclick='showPage(\"logs\",this)'>Logi</button>"
...
"<div class='card-title' style='margin:0'>Logi systemowe</div>"
"<div>"
"<button class='btn' style='margin:0;padding:6px 12px;background:#22263a;color:#e2e8f0;font-size:.8rem' onclick='toggleAutoLogs()'>"
"<span id='log-auto-label'>Auto: ON</span></button>"
"<button class='btn' style='margin:0 0 0 8px;padding:6px 12px;background:#22263a;color:#e2e8f0;font-size:.8rem' onclick='clearLogs()'>Wyczysc</button>"
"</div></div>"
"<pre id='log-output' style='background:#0a0c12;border:1px solid #2e3352;border-radius:8px;"
"padding:12px;font-size:.72rem;line-height:1.5;color:#a5b4cf;overflow:auto;max-height:60vh;"
"white-space:pre-wrap;word-break:break-all;margin:0;font-family:monospace'>Ladowanie...</pre>"
"</div></div>"
"</main>"
"<script>"
"let otaPoller=null;"
...
"const tb=document.querySelector('.tab[data-page=\"'+name+'\"]');if(tb)tb.classList.add('active');}"
"if(location.hash!=='#'+name)location.hash=name;"
"if(name==='dashboard')fetchAll();"
"if(name==='update')fetchStatus();"
"if(name==='logs')startLogs();else if(typeof logTimer!=='undefined'&&logTimer){clearInterval(logTimer);logTimer=null;}"
"}"
...
"let logTimer=null;let logAuto=true;"
"function stripAnsi(s){return s.replace(/\\x1b\\[[0-9;]*m/g,'');}"
"async function fetchLogs(){"
"try{const r=await fetch('/api/logs');let t=await r.text();"
"t=stripAnsi(t);"
"const el=document.getElementById('log-output');if(!el)return;"
"const atBottom=el.scrollHeight-el.scrollTop-el.clientHeight<40;"
"el.textContent=t||'(brak logow)';"
"if(atBottom)el.scrollTop=el.scrollHeight;"
"}catch(e){}}"
"function startLogs(){fetchLogs();if(logTimer)clearInterval(logTimer);"
"if(logAuto)logTimer=setInterval(fetchLogs,2000);}"
"function toggleAutoLogs(){logAuto=!logAuto;"
"document.getElementById('log-auto-label').textContent='Auto: '+(logAuto?'ON':'OFF');"
"if(logAuto){startLogs();}else if(logTimer){clearInterval(logTimer);logTimer=null;}}"
"async function clearLogs(){try{await fetch('/api/logs/clear',{method:'POST'});"
"document.getElementById('log-output').textContent='(wyczyszczono)';}catch(e){}}"
"</script></body></html>";
