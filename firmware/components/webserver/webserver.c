...
"async function aesCbcDecrypt(keyHex,ivBytes,dataBytes){const kb=hexToBytes(keyHex);const k=await crypto.subtle.importKey('raw',kb,{name:'AES-CBC'},false,['decrypt']);try{const pt=await crypto.subtle.decrypt({name:'AES-CBC',iv:ivBytes},k,dataBytes);return new Uint8Array(pt)}catch(e){const blocks=Math.floor(dataBytes.length/16);const out=new Uint8Array(blocks*16);let prev=ivBytes;for(let bi=0;bi<blocks;bi++){const blk=dataBytes.slice(bi*16,bi*16+16);const dec=await crypto.subtle.decrypt({name:'AES-CBC',iv:new Uint8Array(16)},k,appendZero(blk)).catch(()=>null);if(!dec)return null;const d=new Uint8Array(dec).slice(0,16);for(let j=0;j<16;j++)out[bi*16+j]=d[j]^prev[j];prev=blk}return out}}"
"function appendZero(b){const o=new Uint8Array(b.length+16);o.set(b);return o}"
"async function amiplusDecrypt(b){const id=meterId(b).toLowerCase();const key=METER_KEYS[id];if(!key)return null;const iv=new Uint8Array(16);for(let i=0;i<8;i++)iv[i]=b[2+i];for(let i=8;i<16;i++)iv[i]=b[11];let ci=-1;for(let i=10;i<16;i++)if(b[i]===0x7A){ci=i;break}if(ci<0)return null;const encStart=ci+5;const enc=b.slice(encStart,encStart+Math.floor((b.length-encStart)/16)*16);const dec=await aesCbcDecrypt(key,iv,enc);if(!dec||dec[0]!==0x2F||dec[1]!==0x2F)return null;return parseDifVif(dec.slice(2))}"
"async function decodeFrame(hex){const b=hexToBytes(hex);if(b.length<10)return null;const o={id:meterId(b),manufacturer:manuf(b),version:b[8],medium:b[9],rssi:0,fields:[]};if(o.manufacturer==='SAP'&&o.medium===0x01){o.type='izar';const r=izarDecode(b);if(r)o.fields.push({field:'total_m3',value:r.total_m3,unit:'m3'})}else if(o.medium===0x02){o.type='amiplus';const recs=await amiplusDecrypt(b);if(recs){let ve=0,vp=0,vv=0;for(const r of recs){if(r.quantity==='Energy'&&!r.tariff&&ve<1){o.fields.push({field:'energia_kwh',value:r.value,unit:'kWh'});ve++}else if(r.quantity==='Power'&&vp<1){o.fields.push({field:'moc_kw',value:r.value,unit:'kW'});vp++}else if(r.quantity==='Voltage'&&vv<3){o.fields.push({field:'napiecie_l'+(vv+1)+'_v',value:r.value,unit:'V'});vv++}}}}else if(o.manufacturer==='APA'){o.type='apator162'}else if(o.medium===0x03){o.type='gas'}else{o.type='unknown'}return o}"
"async function saveKey(){const id=document.getElementById('keyid').value.trim();const key=document.getElementById('keyval').value.trim();const msg=document.getElementById('keymsg');if(!id||key.length!==32){msg.className='msg err';msg.textContent='Podaj ID i klucz 32 hex';return}msg.className='msg ok';msg.textContent='Zapisywanie...';try{await fetch('/api/config/meter',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id,type:'amiplus',key:key})});METER_KEYS[id.toLowerCase()]=key;msg.textContent='Zapisano klucz dla '+id;document.getElementById('keyval').value='';fetchMeters()}catch(e){msg.className='msg err';msg.textContent='Blad zapisu'}}"
"async function fetchFrames(){try{await loadMeterKeys();const fr=await(await fetch('/api/frames')).json();const byId={};for(const f of fr){const d=await decodeFrame(f.hex);if(!d)continue;if(!byId[d.id]||f.ts>byId[d.id]._ts){d.rssi=f.rssi;d._ts=f.ts;byId[d.id]=d}}return Object.values(byId)}catch(e){return[]}}"
