...
"async function loadSavedMeters(){const el=document.getElementById('saved-list');if(!el)return;try{const cfg=await(await fetch('/api/config')).json();window._savedMeters=cfg.meters||[];if(!cfg.meters||!cfg.meters.length){el.innerHTML='<div class=\"empty\">Brak zapisanych licznikow</div>';return}el.innerHTML=cfg.meters.map(function(m){var k=(m.key&&m.key.length===32)?(m.key.substring(0,4)+'...'+m.key.substring(28)):'(brak)';return '<div style=\"padding:8px;border:1px solid var(--bd);border-radius:8px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;gap:8px\">'+'<div style=\"flex:1;min-width:0\"><div style=\"font-weight:600\">'+(m.name||m.id)+'</div>'+'<div style=\"font-size:.75rem;color:var(--mut)\">ID: '+m.id+' &middot; klucz: '+k+'</div></div>'+'<div style=\"display:flex;gap:6px\"><button class=\"btn\" style=\"margin:0;padding:4px 10px;font-size:.8rem\" onclick=\"editMeter(\''+m.id+'\')\">Edytuj</button>'+'<button class=\"btn\" style=\"margin:0;padding:4px 10px;font-size:.8rem;color:#e66\" onclick=\"deleteMeter(\''+m.id+'\')\">Usun</button></div></div>'}).join('')}catch(e){el.innerHTML='<div class=\"empty\">Blad ladowania</div>'}}"
