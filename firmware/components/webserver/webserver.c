...
"<span id='log-auto-label'>Auto-odswiezanie: ON</span></button>"
...
"white-space:pre-wrap;word-break:break-all;margin:0;font-family:monospace'>Ladowanie logow...</pre>"
"</div></div>"
"</main>"
"<script>"
"let otaPoller=null;"
...
"logAuto=!logAuto;"
"document.getElementById('log-auto-label').textContent='Auto-odswiezanie: '+(logAuto?'ON':'OFF');"
"if(logAuto){startLogs();}else if(logTimer){clearInterval(logTimer);logTimer=null;}"
"}"

"async function clearLogs(){"
"try{await fetch('/api/logs/clear',{method:'POST'});"
"document.getElementById('log-output').textContent='(wyczyszczono)';"
"}catch(e){}}"
