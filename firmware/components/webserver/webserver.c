...
"<div style='color:#64748b;font-size:.85rem'>ESP32-C6 &middot; SIH wMbus Reader</div>"
"</div>"
"<div class='card'>"
...
"<button class='btn btn-github' onclick='otaGithub()'>Aktualizuj z GitHub (najnowsza)</button>"
...
"try{const r=await fetch('https://api.github.com/repos/smartinhome/SIHOS17/releases?per_page=1');"
"const arr=await r.json();"
"const d=Array.isArray(arr)?arr[0]:null;"
"if(!d||!d.assets)return null;"
"const asset=d.assets.find(a=>a.name==='sih-wmbus-reader.bin');"
...
"const fv=document.getElementById('fw-ver');if(fv)fv.textContent=d.version||'-';"
...
"function fmtUptime(ms){let s=Math.floor(ms/1000);"
...
"msg.className='msg ok';msg.textContent='Pobieranie info o wersji...';"
"const url=await getLatestUrl();"
"if(!url){msg.className='msg err';msg.textContent='Nie mozna pobrac URL z GitHub';return;}"
"msg.textContent='Wysylanie OTA: '+url;"
"prog.style.display='block';"
"try{await fetch('/api/ota/url',{method:'POST',"
"headers:{'Content-Type':'application/json'},"
