...
"function removeBlockCrc(d){const out=[];for(let i=0;i<10&&i<d.length;i++)out.push(d[i]);let pos=12;while(pos<d.length){for(let i=0;i<16&&pos+i<d.length;i++)out.push(d[pos+i]);pos+=18}return new Uint8Array(out)}"
