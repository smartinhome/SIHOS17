...
"async function fetchSystem(){const el=document.getElementById('sysinfo');if(!el)return;try{const s=await(await fetch('/api/system')).json();"
...
"function bar(pct,col){return '<div style=\"background:var(--bd);border-radius:6px;height:8px;overflow:hidden;margin-top:4px\"><div style=\"width:'+pct+'%;height:100%;background:'+col+'\"></div></div>'}"
"const heapCol=s.heap_used_pct>80?'#e66':(s.heap_used_pct>60?'#e9a':'var(--acc)');"
"const tempCol=s.temp_c>70?'#e66':(s.temp_c>55?'#e9a':'var(--acc)');"
"const rows=[];"
"rows.push('<div class=\"mf\"><div class=\"k\">Uklad</div><div class=\"val\">'+s.chip_model+' rev'+s.chip_rev+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">CPU</div><div class=\"val\">'+s.cpu_freq_mhz+' MHz &middot; '+s.cpu_cores+' rdzen'+(s.cpu_cores>1?'ie':'')+'</div></div>');"
"if(s.temp_ok)rows.push('<div class=\"mf\"><div class=\"k\">Temperatura</div><div class=\"val\" style=\"color:'+tempCol+'\">'+s.temp_c.toFixed(1)+' &deg;C</div></div>');"
"rows.push('<div class=\"mf\" style=\"flex-direction:column;align-items:stretch\"><div style=\"display:flex;justify-content:space-between\"><div class=\"k\">RAM (heap)</div><div class=\"val\">'+fmtBytes(s.heap_total-s.heap_free)+' / '+fmtBytes(s.heap_total)+' ('+s.heap_used_pct+'%)</div></div>'+bar(s.heap_used_pct,heapCol)+'</div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">RAM min. wolne</div><div class=\"val\">'+fmtBytes(s.heap_min)+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">Najwiekszy blok</div><div class=\"val\">'+fmtBytes(s.heap_largest)+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">Flash</div><div class=\"val\">'+fmtBytes(s.flash_size)+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">Partycja app</div><div class=\"val\">'+fmtBytes(s.app_part_size)+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">Zadania (taski)</div><div class=\"val\">'+s.task_count+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">Uptime</div><div class=\"val\">'+fmtUpt(s.uptime_s)+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">Ostatni restart</div><div class=\"val\">'+s.reset_reason+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">MAC</div><div class=\"val\" style=\"font-family:monospace;font-size:.8rem\">'+s.mac+'</div></div>');"
"const feat=[];if(s.features.wifi)feat.push('WiFi');if(s.features.bt)feat.push('BT');if(s.features.ieee802154)feat.push('802.15.4');"
"rows.push('<div class=\"mf\"><div class=\"k\">Radia</div><div class=\"val\">'+feat.join(', ')+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">ESP-IDF</div><div class=\"val\">'+s.idf_ver+'</div></div>');"
"rows.push('<div class=\"mf\"><div class=\"k\">Kompilacja</div><div class=\"val\" style=\"font-size:.8rem\">'+s.compile_time+'</div></div>');"
"el.innerHTML=rows.join('')}catch(e){el.innerHTML='<div class=\"empty\">Blad ladowania danych systemowych</div>'}}"
