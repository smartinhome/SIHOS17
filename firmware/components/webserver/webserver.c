...
"function amiplusDecode(b){const payload=b.slice(16);const recs=parseDifVif(payload);const out=[];let ve=0,vp=0;for(const r of recs){if(r.quantity==='Energy'&&!r.tariff&&ve<1){out.push({field:'energia_kwh',value:r.value,unit:'kWh'});ve++}else if(r.quantity==='Power'&&vp<1){out.push({field:'moc_kw',value:r.value,unit:'kW'});vp++}else if(r.quantity==='Voltage'){out.push({field:'napiecie_v',value:r.value,unit:'V'})}}return out}"
