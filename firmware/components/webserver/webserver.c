...
"async function fetchMeters(){try{const m=await(await fetch('/api/meters')).json();const dm=document.getElementById('dash-meters'),ml=document.getElementById('meters-list');if(!m.length){dm.innerHTML='<div class=\"empty\">Oczekiwanie na ramki wMbus...</div>';ml.innerHTML='<div class=\"empty\">Brak wykrytych licznikow</div>';return}const html=m.map(meterCard).join('');dm.innerHTML=html;ml.innerHTML=html}catch(e){}}"
